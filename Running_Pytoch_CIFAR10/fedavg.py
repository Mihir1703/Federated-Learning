...
logging.basicConfig(filename='logs/skin-cancer/fedavg.log', level=logging.INFO,
...
# test_set = CIFAR10('../.data', train=False, download=True, transform=transform_train)

train_set = torchvision.datasets.ImageFolder(root="data/chest_xray/train", transform=transform_train)
test_set = torchvision.datasets.ImageFolder(root="data/chest_xray/test", transform=transform_train)
...
class ResNet50(ImageClassificationBase):
    def __init__(self, block=ResidualBlock, layers=None, num_classes=2):
        layers = [3, 4, 6, 3]
...
class GlobalModel:
    def __init__(self, n_clients, communication_rounds, num_epochs_per_round):
        ...
        self.clients = [Client(
            ...
            client_id=i,
            device='cuda:1',
            train_loader=self.dataloaders[i],
    ...
    def prepare_data(self, train_set, test_set):
        ...
        for i in range(self.n_clients):
            client_data.append(torch.utils.data.Subset(train_set, np.arange(i, len(train_set), self.n_clients)))

        self.dataloaders = []
        for data in client_data:
            self.dataloaders.append(DataLoader(data, batch_size=64, shuffle=True))
    ...
    def run(self):
        for i in range(self.communication_rounds):
            ...
            test_load = DeviceDataLoader(self.testloader, device='cuda:1')
