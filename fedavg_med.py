...
logging.basicConfig(filename='fedavg.log', level=logging.INFO,
...
train_set = GTSRB('../.data', split="train", download=True, transform=transform_train)
test_set = GTSRB('../.data', split="test", download=True, transform=transform_train)
...
class ResNet50(ImageClassificationBase):
    def __init__(self, block=ResidualBlock, layers=None, num_classes=43):
        layers = [3, 4, 6, 3]
...
class Client:
    def __init__(self, device, train_loader, test_loader, epochs, client_id, lr=0.001, opt_func=None):
        self.device = device
        self.model = ResNet50(num_classes=43)
